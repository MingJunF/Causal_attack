...
## Run an ODE-based Time Attack experiment

QMIX with ODE
```shell
python3 src/main.py --config=timeattack_qmix --env-config=sc2 with env_args.map_name=2s3z num_attack_test=8 num_attack_train=12 num_followup_agents=2 use_ode_model=True
```
VDN with ODE
```shell
python3 src/main.py --config=timeattack_vdn --env-config=sc2 with env_args.map_name=2s3z num_attack_test=8 num_attack_train=12 num_followup_agents=2 pretrain=False use_ode_model=True
```
QPLEX with ODE
```shell
python3 src/main.py --config=timeattack_qplex --env-config=sc2 with env_args.map_name=2s3z num_attack_test=8 num_attack_train=12 num_followup_agents=2 pretrain=False use_ode_model=True
```
