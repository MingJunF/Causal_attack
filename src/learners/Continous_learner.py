...
class CausalQLearner:
    def __init__(self, mac, scheme, logger, args, qdifference_transformer, planning_transformer,obs_predictor):
        self.args = args
        self.mac = mac
        ...
        # === Observation Predictor for Continuous Follow-up Attack ===
        self.obs_predictor = obs_predictor
        self.optimizer_obs_predictor = optim.Adam(self.obs_predictor.parameters(), lr=args.lr)
        self.log_obs_pred_loss = 0.0
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ######################################################
        # 0) 先在该 batch 上训练 Observation Predictor
        # 收集 (o_t, a_t, o_{t+1}) 进行监督学习
        # 使用 forced_actions (实际执行的行为) 而不是 actions (原始策略)
        obs_current = batch["obs"][:, :-1]  # [B, T, N, obs_shape]
        obs_next = batch["obs"][:, 1:]       # [B, T, N, obs_shape]
        forced_actions_onehot = batch["forced_actions_onehot"][:, :-1]  # [B, T, N, n_actions]
        
        B, T, N, O = obs_current.shape
        
        # Flatten to joint representation for predictor
        obs_joint_t = self._flatten_joint_obs(obs_current)  # [B, T, N*obs_shape]
        obs_joint_next = self._flatten_joint_obs(obs_next)  # [B, T, N*obs_shape]
        action_joint_t = self._flatten_joint_action(forced_actions_onehot)  # [B, T, N*n_actions]
        
        # Reshape to [B*T, ...]
        obs_joint_t_flat = obs_joint_t.reshape(B*T, -1)
        obs_joint_next_flat = obs_joint_next.reshape(B*T, -1)
        action_joint_t_flat = action_joint_t.reshape(B*T, -1)
        
        # Predictor forward pass
        obs_pred_next = self.obs_predictor(obs_joint_t_flat, action_joint_t_flat)  # [B*T, N*obs_shape]
        
        # MSE loss between predicted and actual next observation
        loss_obs_pred = F.mse_loss(obs_pred_next, obs_joint_next_flat)
        
        # Update predictor
        self.optimizer_obs_predictor.zero_grad()
        loss_obs_pred.backward()
        th.nn.utils.clip_grad_norm_(self.obs_predictor.parameters(), self.args.grad_norm_clip)
        self.optimizer_obs_predictor.step()
        
        self.log_obs_pred_loss = loss_obs_pred.item()
        ...
        with th.no_grad():
            ...
            for _ in range(self.args.attack_duration):
                ...
                # Clamp followup_agents_t indices to valid range to avoid CUDA out-of-bounds
                followup_agents_t_clamped = th.clamp(followup_agents_t.long(), 0, self.args.n_agents - 1)
                
                for agent in range(self.args.num_followup_agents):
                    agent_indices = followup_agents_t_clamped[:, :, agent]
                    chosen_predict_chosen_attacker_q = predict_chosen_attacker_q[batch_indices, time_indices, agent_indices]

                    indi_q_slided[batch_indices, time_indices, agent_indices] = chosen_predict_chosen_attacker_q

                    chosen_predict_attacker_actions = predict_attacker_actions[batch_indices, time_indices, agent_indices]

                    do_actions[batch_indices, time_indices, agent_indices] = chosen_predict_attacker_actions
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            # === 新增：Observation Predictor 日志 ===
            self.logger.log_stat("loss_obs_predictor", self.log_obs_pred_loss, t_env)
            wandb.log({"loss_obs_predictor": self.log_obs_pred_loss}, step=t_env)
    ...
    def save_models(self, path):
        ...
        # === 保存 Observation Predictor ===
        th.save(self.obs_predictor.state_dict(), "{}/obs_predictor.th".format(path))
        th.save(self.optimizer_obs_predictor.state_dict(), "{}/optimizer_obs_predictor.th".format(path))
