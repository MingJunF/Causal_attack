...
class ObsPredictorMLP(nn.Module):
    """
    Observation Predictor: 预测下一步的联合观测
    输入：当前时步的 joint observation + joint action（one-hot 或 embedding）
    输出：下一步 t+1 的预测 joint observation
    
    用于 Continuous Follow-up Attack 中的 counterfactual 推断
    """
    def __init__(self, args):
        super().__init__()
        ...
        # 输入维度：joint observation + joint action
        # obs: [obs_shape], action: [n_actions per agent] (one-hot)
        input_obs_dim = self.obs_shape * self.n_agents
        input_action_dim = self.n_actions * self.n_agents
        total_input_dim = input_obs_dim + input_action_dim
        ...
        self.predictor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(hidden_dim, input_obs_dim)  # 输出：joint next obs
        )
    
    def forward(self, obs_joint, action_joint):
        """
        前向传播
        
        Args:
            obs_joint: [B, obs_shape*n_agents] - joint observation at time t
            action_joint: [B, n_actions*n_agents] - joint action (one-hot encoded) at time t
        
        Returns:
            next_obs_pred: [B, obs_shape*n_agents] - predicted joint observation at time t+1
        """
        # Concat obs and action
        x = torch.cat([obs_joint, action_joint], dim=-1)
        
        # Forward through MLP
        next_obs_pred = self.predictor(x)
