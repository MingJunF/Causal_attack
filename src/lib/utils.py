...
def check_module_cycles(module, visited=None, path=None):
    """
    检查模块是否存在自引用循环
    """
    if visited is None:
    ...
    if module_id in visited:
        cycle_path = ' -> '.join([type(m).__name__ for m in path])
        print(f"[WARN] Cycle detected: {cycle_path} -> {type(module).__name__}")
        return True
    ...
    for name, child in module.named_children():
        if check_module_cycles(child, visited, path.copy()):
            has_cycle = True
    
    return has_cycle

def check_gradients(model, max_norm=10.0):
    """
    检查模型梯度是否正常
    """
    total_norm = 0.0
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            if param_norm > max_norm:
                print(f"[WARN] Large gradient in {name}: {param_norm:.4f}")
