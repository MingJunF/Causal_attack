...
class ParseData(object):
    ...
    def generate_train_val_dataloader(self, features, graphs, time_absolute_split, treatment_split, interference_split, is_train=True):
        ...
        # 修复：使用 drop_last=True 丢弃最后不完整的批次
        decoder_data_loader = Loader(series_decoder_all, 
                                batch_size=self.batch_size * self.num_states, 
                                shuffle=False,
                                drop_last=True,  # ✅ 添加这个
        ...
        decoder_graph_loader = Loader(graph_decoder, 
