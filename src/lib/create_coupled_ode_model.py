...
from lib.utils import print_parameters, check_module_cycles

def create_CoupledODE_model(args):


	# dim related
	input_dim = args.obs_shape + args.n_actions  # 节点输入：obs + action_onehot
	z0_prior = None                               # 可选先验（此处置 None）
	...
	# 2. Edge ODE function
	w_node_to_edge_initial_Edge_NRI = nn.Linear(
		(ode_input_dim + treatment_dim) * 2,
		ode_input_dim
	)
	...
	# ✅ 调试: 验证 Linear 层的参数
	print(f"[DEBUG] w_node_to_edge_initial_Edge_NRI.in_features: {w_node_to_edge_initial_Edge_NRI.in_features}")
	print(f"[DEBUG] w_node_to_edge_initial_Edge_NRI.out_features: {w_node_to_edge_initial_Edge_NRI.out_features}")

	edge_ode_func_net = Edge_NRI(
		node_in_channels=ode_input_dim,
		w_node2edge=w_node_to_edge_initial_Edge_NRI,  # ✅ 确保是 nn.Linear 对象
	...
	# ✅ 验证没有自引用
	for name, module in edge_ode_func_net.named_modules():
		if module is edge_ode_func_net and name != '':
			raise RuntimeError(f"Self-reference detected: {name} points to itself!")
	...
	# ✅ 训练前检查模块循环
	if check_module_cycles(model):
		raise RuntimeError("Model contains circular references!")
